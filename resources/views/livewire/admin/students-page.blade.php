    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        <div class="lg:col-span-2 bg-white rounded-lg shadow p-5">
            ...
            <form wire:submit.prevent="save" class="space-y-4">
                ...
                <div class="pt-2">
                    ...
                    @error('selected_tutor_id') <p class="text-red-600 text-xs mt-2">{{ $message }}</p> @enderror
                </div>
            </form>
        </div>
        ...
        <div class="bg-white rounded-lg shadow p-5">
            ...
            @if(!$show_new_tutor)
                ...
                <input type="text" wire:model.debounce.300ms="tutor_search" placeholder="Ej: 30123456 / padre@mail.com / Pérez"
                ...
                @if($selectedTutor)
                    <div class="mt-3 p-3 rounded-md border bg-blue-50 border-blue-200">
                        <div class="text-sm font-medium text-blue-900">
                            {{ $selectedTutor->last_name }}, {{ $selectedTutor->first_name }}
                        </div>
                        <div class="text-xs text-blue-800">
                            DNI: {{ $selectedTutor->dni ?? '—' }} • Email: {{ $selectedTutor->user?->email ?? '—' }}
                        </div>
                        <button type="button" wire:click="clearTutor" class="mt-2 text-xs text-blue-700 hover:underline">
                            Quitar selección
                        </button>
                    </div>
                @endif
                ...
                @if(count($tutorResults))
                    <div class="mt-3 border rounded-md divide-y">
            ...
            @else
                <div class="space-y-3">
                    ...
                    <p class="text-xs text-gray-500">
                        Nota: el tutor se creará primero y luego se actualizará/creará el alumno dentro de una transacción.
                    </p>
                </div>
            @endif
        </div>
                ...
                @forelse($students as $student)
                    ...
                    <tr class="border-t">
                        ...
                        <td class="px-3 py-2 text-right space-x-2">
                            <button wire:click="edit({{ $student->id }})" class="text-blue-600 text-xs">Editar</button>
                            <button wire:click="delete({{ $student->id }})" class="text-red-600 text-xs">Eliminar</button>
                        </td>
                    </tr>
                @empty
                    <tr>
